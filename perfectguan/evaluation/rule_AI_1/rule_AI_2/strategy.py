...
class Strategy(object):
    ...
    def UpdatePlay(self, curPos, curAction, greaterPos, greaterAction, num_of_player_cards, remain_cards, action_history):  # '3', ['Pair','3','['S3','H3']]
        # # print("UpdatePlay:", curPos, curAction, greaterPos, greaterAction)
        for i in range(4):
            self.restHandsCount[i] = num_of_player_cards[i]
            if self.restHandsCount[i] <= 10:
        ...
        history_len = len(action_history)
        if history_len < 4:
            action = action_history[0]
            action_type = action[0]
            action_rank = action[1]
            self.recordPlayerActions[0][action_type] = [action_rank]
            ...
            for i in range(1,history_len):
                ...
                if action[0] == 'PASS':
                    ...
                    action_rank = 'PASS'
                elif action[0] == 'Bomb' and greater_action[0] != 'Bomb':
                    action_type = greater_action[0]
                    action_rank = 'Bomb'
                else:
                    action_type = action[0]
                    action_rank = action[1]
                    greater_action = action
                if action_type not in self.recordPlayerActions[pos].keys():
                    self.recordPlayerActions[pos][action_type] = [action_rank]
                else:
                    self.recordPlayerActions[pos][action_type].append(action_rank)
        else:
            action_id = -4
            while action_history[action_id][0] == 'PASS':
                action_id -= 1
            greater_action = action_history[action_id]
            for i in range(-4, 0):
                pos = (curPos + i) % 4
                action = action_history[i]
                if action[0] == 'PASS':
        ...
        for key in self.restCardsCount.keys():
            self.restCardsCount[key] = remain_cards[key]
