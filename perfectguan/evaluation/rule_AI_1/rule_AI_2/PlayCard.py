...
import json
import time
from .strategy import Strategy0
...
class PlayCard():
    ...
    def FreePlay(self, handCards, curRank, fullActionList = None):
        # print("Free play handCards:", handCards, "   restHandsCount:", Strategy0.restHandsCount)
        handValue, handActions = CountValue().HandCardsValue(handCards, 0, curRank)
        ## print(handActions)
        #Strategy.SetBeginning(0)
        Strategy0.SetRole(handValue, handActions, curRank)
        Strategy0.makeReviseValues()
        ...
        # beginning
        bestPlay = {}
        if (len(handCards)>=15 or Strategy0.roundStage != 'ending'):
            minValue = 100
            for action in handActions:
                actionValue = CountValue().ActionValue(action, action['type'], action['rank'], curRank) - Strategy0.freeActionRV[action['type']] \
                        - Strategy0.freeActionRV[(action['type'],action['rank'])]
        ...
        ## print(Strategy.freeActionRV[('Pair','Q')])
            ...
            for i in range(0, len(config.cardTypes)):
                ...
                for rank1 in actionList[type]:
                    for card in actionList[type][rank1]:
                        ...
                        thisHandValue += Strategy0.freeActionRV[type]
                        if (type, rank) in Strategy0.freeActionRV.keys():
                            thisHandValue += Strategy0.freeActionRV[(type, rank)]
            ...
            for action in additionalActionList:
                ...
                restValue, restActions = CountValue().HandCardsValue(restCards, 0, curRank)
                restValue += Strategy0.handRV[type]
    ...
    def RestrictedPlay(self, handCards, formerAction, curRank, fullActionList = None):
        # print("Restricted Play handCards:", handCards,"   restHandsCount:", Strategy0.restHandsCount)
        actionList = CreateActionList().CreateList(handCards)
        ...
        Strategy0.SetRole(maxValue, restActions, curRank)
        Strategy0.makeReviseValues()
        maxValue += Strategy0.restrictedActionRV["PASS"]
            ...
            for rank1 in actionList[type]:
                for card in actionList[type][rank1]:
                    ...
                    if (CompareRank().Larger(type, rank, card, formerAction, curRank)):
                        ...
                        thisHandValue = CountValue().ActionValue(action, type, rank, curRank)
                        thisHandValue += Strategy0.restrictedActionRV[type]
                        if (type, rank) in Strategy0.restrictedActionRV.keys():
                            thisHandValue += Strategy0.restrictedActionRV[(type, rank)]
                        ## print(Strategy.actionValueRevise)
                        ## print(rank, card, thisHandValue, restValue)
                        if (thisHandValue < 0): thisHandValue = 0
                        if (thisHandValue + restValue > maxValue or (thisHandValue + restValue == maxValue and \
